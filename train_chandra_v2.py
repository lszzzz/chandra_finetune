...
VIDEO_TOKEN_ID = 151656   # Reference constant; not used in OCR data paths
BOS_TOKEN_ID = 151643     # Reference constant; included for completeness
EOS_TOKEN_ID = 151645     # Reference constant; included for completeness
...
# ---------------------------------------------------------------------------
# 2b. Lazy Vision Data Collator
# ---------------------------------------------------------------------------

def _resolve_image(img) -> Image.Image:
    """Decode *img* to ``PIL.Image`` regardless of storage format.

    Supports PIL.Image (passthrough), str/Path (file open),
    and HuggingFace Image feature dicts with "bytes" or "path" keys.
    """
    ...
    raise TypeError(
        f"Cannot resolve image from type {type(img)!r}. "
        "Expected PIL.Image, str/Path, or HuggingFace Image feature dict."
    )


class _DecodingVisionCollator(UnslothVisionDataCollator):
    """Subclass of ``UnslothVisionDataCollator`` that decodes images per-batch.

    WHY SUBCLASS (not wrap):
    ``UnslothSFTTrainer.__init__`` does an explicit
    ``isinstance(data_collator, UnslothVisionDataCollator)`` check from
    ``unsloth_zoo.vision_utils``. When the check fails, Unsloth silently
    replaces the collator with ``DataCollatorWithPadding``, causing:
        ValueError: you provided ['messages']
    Subclassing makes ``isinstance`` return True so Unsloth never touches it.

    HOW IT WORKS:
    The dataset stores images as raw bytes dicts (never decoded PIL), so the
    plain Python list stays compact in RAM. This collator decodes each image
    to PIL just-in-time before delegating to the parent ``__call__``.
    RAM cost at any moment: O(batch_size), not O(dataset_size).
    """

    def __init__(self, model, tokenizer, **collator_kwargs):
        super().__init__(model, tokenizer, **collator_kwargs)

    def __call__(self, batch):
        """Decode image bytes dicts to PIL then delegate to parent."""
        decoded = []
        for item in batch:
            new_messages = []
            for msg in item["messages"]:
                ...
                for part in msg["content"]:
                    if part.get("type") == "image":
                        new_content.append({**part, "image": _resolve_image(part["image"])})
                    else:
                        new_content.append(part)
                new_messages.append({**msg, "content": new_content})
            decoded.append({"messages": new_messages})
        return super().__call__(decoded)
...
def prepare_dataset(
    ...
    def _convert_split(dataset):
        """Convert dataset to a plain Python list storing raw image bytes.

        WHY A PLAIN LIST:
        Unsloth's SFTTrainer inspects the dataset type. HF Dataset and
        torch.utils.data.Dataset objects trigger collator replacement logic.
        A plain Python list is opaque — Unsloth cannot inspect it and always
        calls the collator you passed in.

        WHY RAW BYTES (not PIL):
        A decoded PIL image at ~1500px uses ~12 MB. 80K samples = ~960 GB.
        Raw JPEG bytes for the same image are ~100-500 KB (~50x smaller).
        We store bytes, decode per-batch in _DecodingVisionCollator.
        """
        from datasets import Image as HFImage

        # Cast Image(decode=True) → Image(decode=False) so iteration gives
        # raw {"bytes": ..., "path": ...} dicts instead of PIL objects.
        features = dataset.features.copy()
        changed = False
        img_cols = ["images"] if fmt == "finevision" else [args.image_column]
        for col in img_cols:
            if col in features and isinstance(features[col], HFImage) and features[col].decode:
                features[col] = HFImage(decode=False)
                changed = True
                log.info("Cast image column '%s' to decode=False for compact storage.", col)
        if changed:
        ...
        for sample in dataset:
            if fmt == "finevision":
                img_ref = sample["images"][0]
                text_entry = sample["texts"][0]
                ground_truth = (
                    text_entry.get("assistant", "") if isinstance(text_entry, dict)
                    else str(text_entry)
                )
            else:
                img_ref = sample[args.image_column]
                ground_truth = sample[args.text_column]

            # img_ref is {"bytes": ..., "path": ...} — decoded by _DecodingVisionCollator
            result.append({
                "messages": [
                    {
                        "role": "user",
                        "content": [
                            {"type": "text",  "text": args.ocr_instruction},
    ...
    log.info("Converting %d train samples to list (raw image bytes)...", len(train_ds))
    train_conv = _convert_split(train_ds)

    eval_conv = None
    if eval_ds is not None:
        log.info("Converting %d eval samples to list (raw image bytes)...", len(eval_ds))
        eval_conv = _convert_split(eval_ds)

    log.info(
        "Conversion complete (plain list, bytes decoded per batch): train=%d  eval=%s",
        len(train_conv), len(eval_conv) if eval_conv else "None",
    )
...
def _make_compute_metrics(tokenizer, max_samples: int = 64, seed: int = 3407):
    ...
    Args:
        ...
        seed: Random seed for reproducible sample selection via numpy's
              default_rng. Uses ``args.seed`` so that the same eval subset
              is chosen at every evaluation step across all runs, making
              CER/WER values directly comparable between experiments.
    """
    pad_id = int(tokenizer.pad_token_id if tokenizer.pad_token_id is not None else 0)
    vocab_sz = int(getattr(tokenizer, "vocab_size", VOCAB_SIZE))

    # FIX: Use a seeded, isolated RNG instead of np.random.choice so that
    # sample selection is reproducible and does not pollute global numpy state.
    ...
    def compute_metrics(eval_pred):
        ...
        # Cap samples to avoid O(N * seq_len) Python decode overhead.
        # Uses the seeded RNG so the same subset is picked every eval step.
        n = pred_ids.shape[0]
        if n > max_samples:
        ...
        # Trim trailing padding columns to reduce decode work.
...
def build_trainer(
    ...
    if use_early_stopping:
        ...
        # FIX: Warn when using a metric where lower-is-better assumption may be wrong.
        _LOWER_IS_BETTER_METRICS = {"eval_loss", "loss", "eval_cer", "cer", "eval_wer", "wer"}
        if args.metric_for_best_model not in _LOWER_IS_BETTER_METRICS:
            log.warning(
                "greater_is_better=False is assumed for metric '%s'. "
                "If this metric is higher-is-better (e.g. accuracy, F1), "
                "the trainer will select the worst model. Use a loss/error-rate "
                "metric or adjust greater_is_better manually.",
    ...
    # FIX: Pass args.seed into _make_compute_metrics so that the random
    # subsample of eval sequences is identical across every eval step and
    # every experiment run, making CER/WER values directly comparable.
    ...
    if args.compute_cer_wer and eval_dataset is not None:
        if JIWER_AVAILABLE:
            compute_metrics_fn = _make_compute_metrics(
                tokenizer, max_samples=64, seed=args.seed
            )
            preprocess_logits_fn = _preprocess_logits_for_metrics
            log.info("CER/WER metrics enabled (teacher-forced, via jiwer, seed=%d)", args.seed)
        else:
            log.warning(
    ...
    trainer = SFTTrainer(
        model=model,
        tokenizer=tokenizer,
        data_collator=_DecodingVisionCollator(model, tokenizer, resize=args.longest_edge),
...
def train(args: argparse.Namespace) -> None:
    ...
    # FIX: Report memory stats at end of training using report_memory().
    log.info("Final memory stats: %s", report_memory())
    ...
    # FIX: Use eval_data for the post-training test when available so we
    # measure generalisation rather than memorisation of a training sample.
    log.info("=" * 60)
    log.info("PHASE: Post-training inference test")
    log.info("=" * 60)
    post_test_data = eval_data if eval_data else train_data
    _run_inference_test(model, tokenizer, processor, post_test_data, tag="AFTER training")
...
def _run_inference_test(
    model, tokenizer, processor, converted_dataset, *, tag: str = ""
) -> Optional[str]:
    """Run a single inference sample to verify the model works.

    Uses the first sample from the converted dataset. Compatible with both
    HuggingFace ``Dataset`` objects (Arrow-backed) and plain Python lists.
    """
    FastVisionModel.for_inference(model)

    if converted_dataset is None or len(converted_dataset) == 0:
    ...
    # Search explicitly for both image and text parts by type so that
    # content ordering does not matter.
    test_instruction = OCR_INSTRUCTION
    for part in user_content:
        if part.get("type") == "image":
            # Resolve lazily-stored image reference to PIL at inference time.
            raw = part.get("image")
            test_image = _resolve_image(raw) if raw is not None else None
        elif part.get("type") == "text":
            test_instruction = part.get("text", OCR_INSTRUCTION)
